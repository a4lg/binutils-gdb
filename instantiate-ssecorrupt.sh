...
do_csr_instantiation ()
{
	...
	sed -i "s/^#define CSR_${CSR_UPPER} .*/#define CSR_${CSR_UPPER} $HEXC  \/* Ssecorrupt extension.  *\//" include/opcode/riscv-opc.h
	for V in $PRIV_VERSIONS
	do
		sed -i "s/+${ORIG_ADDR}\\(02573\\|59073\\)/+${ADDR}\\1/" gas/testsuite/gas/riscv/csr-version-$V.d
	done
	sed -i "s/\\.cfi_offset ${CSR}, .*/.cfi_offset ${CSR}, ${DWOFF}	# New in Ssecorrupt extension/" gas/testsuite/gas/riscv/csr-dw-regnums.s
